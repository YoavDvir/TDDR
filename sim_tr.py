...
def compare_tddr():
    ...
    parser = argparse.ArgumentParser(description ='run the TDDR from command line.')
    parser.add_argument('trials',
                        metavar ='trials',
                        type = int,
                        nargs = 1,
                        help ='number of repetitions. Each repetition a new random tree will be created.')

    parser.add_argument('mean_edge_length',
                        metavar ='edge_ml',
                        type = float,
                        nargs = 1,
                        help ='the edge length of the random tree has exponential distribution with this mean edge length')

    parser.add_argument('number_of_leaves',
                        metavar ='leaves_n',
                        type = int,
                        nargs = 1,
                        help ='the number of leaves in the random tree')

    parser.add_argument('number_of_genes',
                        metavar ='genes_n',
                        type = int,
                        nargs = 1,
                        help ='the number of genes in the root genome of the random tree')
    '''
    parser.add_argument('with_dcj',
                        metavar = 'wdcj',
                        type = Boolean,
                        nargs = 1,
                        help ='If True applies DCJ. Need UNIMOG package (with JAVA) to be installed. ')
    '''
    parser.add_argument(dest ='rep',
                        action ='store_const',
                        const = tddtr_obj.repetitions,
                        help ='compute the parameters theta and alpha')

    args = parser.parse_args()
    print('test',args.trials[0],args.mean_edge_length[0],args.number_of_leaves[0],args.number_of_genes[0])
    return args.rep(args.trials[0],args.mean_edge_length[0],args.number_of_leaves[0],args.number_of_genes[0])
...
#_______________________________________________________________________________________________________________________
    ...
    with_unimog = True              # True to apply the dcj method. The UNIMOG package and JAVA should be installed
...
#_______________________________________________________________________________________________________________________
    ...
    leavesgnm = []                  # Set of COGs for each taxon. Without repetitions
    pairs = []                      # Set intersections betweens each pair of taxa
    log_leaves_size = []            # list of log of number of distinct COGS for each tax\on
    ...
    errors = []                     # For each taxon the number of distances of triplets with that leaf with negative
                                    # value
    sum_errors = []                 # For each taxon the sum of negative values in triplets that containes this leaf.
    ...
    def unimog_createsmatrix(self):
        try:
            ...
            print("no file dcj_matrix.txt")
        finally:
            try:
                os.remove('UNIMOGfile.txt')
            except OSError:
                print("no file UNIMOGfile.txt")
            finally:
                ...
                try:
                    ...
                    print("cannot run UniMoG.jar")
                    self.with_unimog = False
                    return
    ...
    def init_repetitions(self):
        ...
        return
    def repetitions(self, trials, mean_edge_length, number_of_leaves,number_of_genes):

        self.trials = trials
        self.mean_edge_length = mean_edge_length
        self.leaves_number = number_of_leaves
        self.n0 = number_of_genes
        #self.with_unimog = with_dcj
        ...
        return self.mean_success_rate
print("mean_success_rate of TDDR:",compare_tddr())
#tddtr_obj = TDDTR()
#tddtr_obj.repetitions(tddtr_obj.trials,
#            tddtr_obj.mean_edge_length,
#            tddtr_obj.number_of_leaves,
#            tddtr_obj.n0,
#            tddtr_obj.with_unimog)
